...
-- Atualizar paióis que têm dragagem ativa para status DRAGANDO
...
-- Atualizar a função execute_status_transition para garantir que o status seja mantido corretamente
...
-- Criar trigger para manter consistência do status quando dragagem for finalizada diretamente
CREATE OR REPLACE FUNCTION public.update_paiol_status_on_dragagem_change()
RETURNS TRIGGER AS $$
BEGIN
  -- Se uma dragagem foi finalizada (data_fim foi adicionada)
...
CREATE TRIGGER trigger_update_paiol_status_on_dragagem_change
  AFTER UPDATE ON public.dragagens
  FOR EACH ROW
  EXECUTE FUNCTION public.update_paiol_status_on_dragagem_change();
