...
-- Remover a função duplicada e manter apenas a versão mais simples
DROP FUNCTION IF EXISTS public.execute_status_transition(uuid, text, uuid, uuid, text, uuid, timestamp with time zone, timestamp with time zone);

-- Verificar se a função principal ainda existe, se não, recriar
